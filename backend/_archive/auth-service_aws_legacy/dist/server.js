...
const hashOpaque = (token) => createHash('sha256').update(token).digest('hex');
let dummyPasswordHash;
function passwordError(password, identity) {
    if (password.length < 12 || password.length > 128)
        return 'Parola 12–128 karakter olmalıdır.';
...
async function validateNewPassword(password, identity) {
    const policyError = passwordError(password, identity);
    if (policyError)
        return { code: 'WEAK_PASSWORD', message: policyError };
    const breachResult = await checkBreachedPassword(password);
    if (breachResult === 'breached')
        return { code: 'WEAK_PASSWORD', message: 'Bu parola bilinen veri sızıntılarında yer alıyor.' };
    if (breachResult === 'unavailable')
        return { code: 'PASSWORD_CHECK_UNAVAILABLE', message: 'Parola güvenlik kontrolü şu anda tamamlanamadı.' };
    return null;
}
const signAccessToken = async (user) => new SignJWT({ email: user.email })
...
async function issueSession(user, existingFamilyId) {
    const client = await db.connect();
    try {
        await client.query('BEGIN');
        const familyId = existingFamilyId ?? (await client.query('INSERT INTO refresh_token_families(user_id) VALUES($1) RETURNING id', [user.id])).rows[0].id;
...
async function createActionToken(userId, kind, ttl) {
...
async function deliverActionLink(email, kind, token) {
    const endpoint = process.env.EMAIL_DELIVERY_WEBHOOK;
    if (!endpoint) {
        if (process.env.NODE_ENV === 'production')
            throw new Error('EMAIL_DELIVERY_WEBHOOK is required in production');
...
async function consumeActionToken(token, kind) {
    const client = await db.connect();
    try {
        await client.query('BEGIN');
        const result = await client.query('SELECT id,user_id FROM account_action_tokens WHERE token_hash=$1 AND kind=$2 AND consumed_at IS NULL AND expires_at > now() FOR UPDATE', [hashOpaque(token), kind]);
...
async function requireUser(request) {
    ...
    if (!userId)
        throw new Error('UNAUTHORIZED');
    const result = await db.query('SELECT id,email,display_name FROM users WHERE id=$1 AND email_verified_at IS NOT NULL', [userId]);
    if (!result.rows[0])
        throw new Error('UNAUTHORIZED');
    return result.rows[0];
}
async function createWebAuthnChallenge(userId, purpose, challenge) {
...
async function consumeWebAuthnChallenge(challenge, purpose) {
    const result = await db.query('UPDATE webauthn_challenges SET consumed_at=now() WHERE challenge=$1 AND purpose=$2 AND consumed_at IS NULL AND expires_at > now() RETURNING id,user_id', [challenge, purpose]);
...
app.post('/v1/auth/register', { config: { rateLimit: { max: 5, timeWindow: '1 hour' } } }, async (request, reply) => {
    ...
    if (passwordValidation)
        return reply.code(passwordValidation.code === 'PASSWORD_CHECK_UNAVAILABLE' ? 503 : 400).send({ error: passwordValidation });
    const passwordHash = await hashPassword(input.password);
    try {
        const result = await db.query('INSERT INTO users(email, display_name, password_hash) VALUES($1,$2,$3) RETURNING id,email', [input.email.toLowerCase(), input.name, passwordHash]);
...
app.post('/v1/auth/email/verify', { config: { rateLimit: { max: 10, timeWindow: '15 minutes' } } }, async (request, reply) => {
    ...
    if (!userId)
        return reply.code(400).send({ error: { code: 'INVALID_OR_EXPIRED_TOKEN', message: 'Doğrulama bağlantısı geçersiz veya süresi dolmuş.' } });
...
app.post('/v1/auth/password-reset/request', { config: { rateLimit: { max: 5, timeWindow: '1 hour' } } }, async (request, reply) => {
    ...
    const result = await db.query('SELECT id FROM users WHERE email=$1', [input.email.toLowerCase()]);
    if (result.rows[0]) {
...
app.post('/v1/auth/password-reset/confirm', { config: { rateLimit: { max: 8, timeWindow: '15 minutes' } } }, async (request, reply) => {
    ...
    if (!userId)
        return reply.code(400).send({ error: { code: 'INVALID_OR_EXPIRED_TOKEN', message: 'Sıfırlama bağlantısı geçersiz veya süresi dolmuş.' } });
    const account = await db.query('SELECT email,display_name FROM users WHERE id=$1', [userId]);
...
app.post('/v1/auth/login', { config: { rateLimit: { max: 8, timeWindow: '15 minutes' } } }, async (request, reply) => {
    ...
    const result = await db.query('SELECT id,email,password_hash,email_verified_at FROM users WHERE email=$1', [input.email.toLowerCase()]);
    const user = result.rows[0];
    const valid = await verifyPassword(input.password, user?.password_hash);
    if (!valid)
        return reply.code(401).send({ error: { code: 'INVALID_CREDENTIALS', message: 'E-posta veya şifre hatalı.' } });
    if (!user.email_verified_at)
        return reply.code(403).send({ error: { code: 'EMAIL_VERIFICATION_REQUIRED', message: 'E-posta doğrulaması gerekli.' } });
...
app.post('/v1/auth/refresh', { config: { rateLimit: { max: 30, timeWindow: '15 minutes' } } }, async (request, reply) => {
    ...
    try {
        await client.query('BEGIN');
        const token = await client.query('SELECT t.id,t.family_id,u.id AS user_id,u.email,t.consumed_at,t.expires_at,f.revoked_at FROM refresh_tokens t JOIN refresh_token_families f ON f.id=t.family_id JOIN users u ON u.id=f.user_id WHERE t.token_hash=$1 FOR UPDATE', [hashOpaque(input.refreshToken)]);
        const row = token.rows[0];
        if (!row || row.consumed_at || row.revoked_at || row.expires_at <= new Date()) {
            if (row)
                await client.query('UPDATE refresh_token_families SET revoked_at=now() WHERE id=$1', [row.family_id]);
            await client.query('COMMIT');
...
app.post('/v1/auth/logout', async (request, reply) => {
    ...
    await db.query(`UPDATE refresh_token_families
...
app.post('/v1/auth/passkeys/registration/options', async (request, reply) => {
    try {
        ...
        const existing = await db.query('SELECT credential_id,transports FROM webauthn_credentials WHERE user_id=$1', [user.id]);
        const options = await generateRegistrationOptions({ rpName: 'TurkSquare', rpID, userID: new TextEncoder().encode(user.id), userName: user.email, userDisplayName: user.display_name, attestationType: 'none', excludeCredentials: existing.rows.map((item) => ({ id: item.credential_id, transports: item.transports })), authenticatorSelection: { residentKey: 'required', userVerification: 'required' } });
        await createWebAuthnChallenge(user.id, 'registration', options.challenge);
        return { data: options };
    }
    catch (_) {
        return reply.code(401).send({ error: { code: 'UNAUTHORIZED', message: 'Oturum doğrulanamadı.' } });
    }
});
...
app.post('/v1/auth/passkeys/registration/verify', async (request, reply) => {
    try {
        ...
        const challenges = await db.query('SELECT challenge FROM webauthn_challenges WHERE user_id=$1 AND purpose=\'registration\' AND consumed_at IS NULL AND expires_at > now()', [user.id]);
        for (const item of challenges.rows) {
            try {
                ...
                if (!verified.verified || !verified.registrationInfo)
                    continue;
                if (!await consumeWebAuthnChallenge(item.challenge, 'registration'))
                    break;
...
app.post('/v1/auth/passkeys/authentication/verify', { config: { rateLimit: { max: 12, timeWindow: '15 minutes' } } }, async (request, reply) => {
    try {
        const input = webauthnSchema.parse(request.body);
        const response = input.credential;
        const dbCredential = await db.query('SELECT c.user_id,u.email,c.credential_id,c.public_key,c.counter,c.transports FROM webauthn_credentials c JOIN users u ON u.id=c.user_id WHERE c.credential_id=$1 AND u.email_verified_at IS NOT NULL', [response.id]);
        const credential = dbCredential.rows[0];
        if (!credential)
            return reply.code(401).send({ error: { code: 'PASSKEY_FAILED', message: 'Passkey doğrulanamadı.' } });
        const challenges = await db.query('SELECT challenge FROM webauthn_challenges WHERE purpose=\'authentication\' AND consumed_at IS NULL AND expires_at > now()');
        for (const item of challenges.rows) {
            try {
                const verified = await verifyAuthenticationResponse({ response, expectedChallenge: item.challenge, expectedOrigin, expectedRPID: rpID, requireUserVerification: true, credential: { id: credential.credential_id, publicKey: new Uint8Array(credential.public_key), counter: credential.counter, transports: credential.transports } });
                if (!verified.verified)
                    continue;
                if (!await consumeWebAuthnChallenge(item.challenge, 'authentication'))
                    break;
