...
type FeedRow = { id: string; created_at: Date; body: string; location_label: string | null; author_name: string; likes: string; comments: string; is_liked: boolean; media: FeedMediaRow[] | null; poll: FeedPollRow | null; purpose: string; is_author: boolean; traveler: FeedTravelerRow | null };
...
app.get('/v1/community/feed', async (request, reply) => {
  try {
    ...
    return { data: await Promise.all(page.map(async (p) => ({ id:p.id, authorName:p.author_name, location:p.location_label ?? '', createdAtLabel:p.created_at.toISOString(), message:p.body, likes:Number(p.likes), comments:Number(p.comments), isLiked:p.is_liked, isAuthor:p.is_author, purpose: FEED_PURPOSE[p.purpose] ?? 'standard', travelerMatch: p.traveler ? { ...p.traveler, travelAt: new Date(p.traveler.travelAt).toISOString() } : null, media: await feedMediaJson(p.media), poll: feedPollJson(p.poll) }))), meta: { nextCursor: next } };
