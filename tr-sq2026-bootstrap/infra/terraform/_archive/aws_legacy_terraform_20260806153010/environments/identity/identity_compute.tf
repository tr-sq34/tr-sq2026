...
resource "aws_iam_role_policy" "github_identity_deploy" {
  ...
  policy = jsonencode({
    ...
    Statement = [{
      ...
      Effect = "Allow"
      # Releases use an immutable Git-SHA tag.  Read it first so a retry can
      # reuse an existing verified image instead of attempting an overwrite.
      ...
      Effect   = "Allow"
      Action   = ["ecs:DescribeServices", "ecs:DescribeTaskDefinition", "ecs:DescribeTasks", "ecs:ListTasks", "ecs:RegisterTaskDefinition", "ecs:RunTask", "ecs:UpdateService", "logs:FilterLogEvents", "ec2:DescribeVpcEndpoints", "ec2:DescribeRouteTables", "ec2:DescribeSubnets", "ec2:DescribeVpcs", "ec2:DescribeDhcpOptions"]
      Resource = "*"
      }, {
...
resource "aws_iam_role_policy" "identity_task_secrets" {
  ...
  policy = jsonencode({
    ...
    Statement = concat([{
      ...
      Resource = [try(aws_lambda_function.email_relay[0].arn, ""), try(aws_lambda_function.password_safety[0].arn, "")]
      }, {
...
resource "aws_ecs_task_definition" "identity" {
  ...
  container_definitions = jsonencode([{
    ...
    environment = [
      ...
      { name = "EMAIL_RELAY_FUNCTION_NAME", value = try(aws_lambda_function.email_relay[0].function_name, "") },
      { name = "PASSWORD_SAFETY_FUNCTION_NAME", value = try(aws_lambda_function.password_safety[0].function_name, "") },
      { name = "GATEWORK_COMMUNITY_AUDIENCE", value = "turksquare-community-gatework" },
    ]
    secrets = concat(
      [
        # Only long-lived runtime configuration belongs in the ECS secret
        # contract.  GATEWORK_BOOTSTRAP_OWNER_EMAIL is deliberately excluded:
        # it is a one-time, optional owner-seeding value and is removed after
        # bootstrap.  Keeping it here makes every later task startup fail when
        # the key is (correctly) removed from Secrets Manager.
