...
resource "aws_iam_role_policy" "github_community_deploy" {
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      {
        Effect   = "Allow"
        # DescribeImages is used to verify the immutable, one-time bootstrap
        # fallback image before a release attempts to create it. Keeping this
        # read permission on the two deployment repositories lets the workflow
        # distinguish a harmless concurrent seed from an actual ECR failure.
        ...
        Resource = [aws_ecr_repository.community.arn, aws_ecr_repository.gatework.arn]
      },
      {
        Effect   = "Allow"
      ...
      },
      {
        Effect    = "Allow"
        Action    = ["iam:PassRole"]
        Resource  = [aws_iam_role.community_execution.arn, aws_iam_role.community_task.arn, aws_iam_role.gatework_execution.arn, aws_iam_role.gatework_task.arn]
